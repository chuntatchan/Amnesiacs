	...
	void Update ()
	{
		...
		if (Input.GetMouseButtonDown(0))
		{
			...
			if (frontmostRaycastHit.nothingClicked == false)
			{
				...
				// Displays the name of the object left clicked
				if (showDebug) Debug.Log("Left-Clicked on: " + gameObject.name);
			}
		}
		...
		else if (Input.GetMouseButtonDown(1))
		{
			...
			if (frontmostRaycastHit.nothingClicked == false)
			{
				...
				// Sends the frontmostRaycast to a function called OnRightClick in a script attached to whatever the rightClickedObject is.
				rightClickedObject.SendMessage("OnRightClick", frontmostRaycastHit, SendMessageOptions.DontRequireReceiver);

				// Displays the name of the object rightt clicked
				if (showDebug) Debug.Log("Right-Clicked on: " + gameObject.name);
			}
		}
	}
